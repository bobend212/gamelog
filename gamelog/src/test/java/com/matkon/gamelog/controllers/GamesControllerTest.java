...
class GamesControllerTest {
    ...
    void getLibraryGamesTest() throws Exception {
        ...
        when(gamesService.getLibraryGames(0, 8, "ALL", "test"))
                .thenReturn(gamePage);
        ...
        verify(gamesService).getLibraryGames(anyInt(), anyInt(), eq("ALL"), eq("test"));
    }
    ...
    void getWishlistGamesTest() throws Exception {
        ...
        when(gamesService.getWishlistGames(0, 8, "test"))
                .thenReturn(gamePage);
        ...
        verify(gamesService).getWishlistGames(anyInt(), anyInt(), eq("test"));
    }

    @Test
    void getWishlistGamesDashboardTest() throws Exception {
        WishlistGameForTableDto game = new WishlistGameForTableDto();
        WishlistGameForTableDto game2 = new WishlistGameForTableDto();
        List<WishlistGameForTableDto> games = List.of(game, game2);
        Page<WishlistGameForTableDto> gamePage = new PageImpl<>(games);

        when(gamesService.getWishlistGamesDashboard(0, 50, "releaseDate,asc", ReleaseFilter.ALL))
                .thenReturn(gamePage);
        ...
        mockMvc.perform(get("/api/games/wishlist/dashboard")
                        ...
                        .param("release", ReleaseFilter.ALL.name()))
                .andExpect(status().isOk());

        verify(gamesService).getWishlistGamesDashboard(anyInt(), anyInt(), eq("releaseDate,asc"), eq(ReleaseFilter.ALL));
    }

    @Test
    void searchGamesTest() throws Exception {

        GameSearchDto dto = new GameSearchDto(123L, "Test Game", "http://image.jpg", LocalDate.of(2023, 6, 15));
        List<GameSearchDto> dtoList = List.of(dto);

        when(gamesService.searchGames("test")).thenReturn(dtoList);

        mockMvc.perform(get("/api/games/search")
                        .param("query", "test"))
                .andExpect(status().isOk());

        verify(gamesService).searchGames("test");
    }
    ...
    void addGameToLibrarySuccessTest() throws Exception {
        ...
        when(gamesService.saveGameToDatabase(rawgId, GameStatus.BACKLOG))
                .thenReturn(result);
    ...
    void addGameToWishlistSuccessTest() throws Exception {
        ...
        when(gamesService.saveGameToDatabase(rawgId, GameStatus.WISHLIST))
                .thenReturn(result);
    ...
    void updateGameSuccessTest() throws Exception {
        ...
        when(gamesService.updateGame(eq(gameId), any(GameUpdateRequest.class)))
    ...
    void syncLibraryGamesTest() throws Exception {
        ...
        when(gamesService.syncLibraryGames(eq(GameStatus.WISHLIST)))
