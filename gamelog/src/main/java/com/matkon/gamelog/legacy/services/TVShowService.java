package com.matkon.gamelog.legacy.services;

import com.fasterxml.jackson.databind.JsonNode;
import com.fasterxml.jackson.databind.ObjectMapper;
import com.matkon.gamelog.legacy.data.sync.ChangeDetail;
import com.matkon.gamelog.legacy.data.sync.FieldChange;
import com.matkon.gamelog.legacy.data.sync.SyncResultDto;
import com.matkon.gamelog.legacy.data.sync.SyncUtils;
import com.matkon.gamelog.legacy.data.tvshow.TVShow;
import com.matkon.gamelog.legacy.data.tvshow.TrackingType;
import com.matkon.gamelog.legacy.data.tvshow.dto.TVShowDto;
import com.matkon.gamelog.legacy.data.tvshow.dto.TVShowListDto;
import com.matkon.gamelog.legacy.data.tvshow.dto.TVShowSaveResultDto;
import com.matkon.gamelog.legacy.data.tvshow.dto.TVShowSearchResultDto;
import com.matkon.gamelog.legacy.data.tvshow.season.Season;
import com.matkon.gamelog.legacy.data.tvshow.season.dto.SeasonDto;
import com.matkon.gamelog.legacy.repos.SeasonRepository;
import com.matkon.gamelog.legacy.repos.TVShowRepository;
...
public class TVShowService {
    ...
    public TVShowService(TVShowRepository seriesRepo,
    ...
    // ---- Search ----
    public List<TVShowSearchResultDto> searchByQuery(String query, String lang, int page) {
        ...
        return response.results.stream()
                .map(r -> new TVShowSearchResultDto(r.id, r.name, parseDate(r.first_air_date), r.poster_path))
                .toList();
    }
    ...
    public TVShowSaveResultDto saveSeries(Long tmdbId, TrackingType TrackingType) {
        ...
        tvShow.setTmdbId(details.id);
        tvShow.setName(details.name);
        tvShow.setFirst_air_date(details.first_air_date);
        tvShow.setNumber_of_episodes(details.number_of_episodes);
        tvShow.setNumber_of_seasons(details.number_of_seasons);
        ...
        try {
            tvShow.setVodProviders(getVodProviders(webClient, tmdbApiKey, tmdbId));
        } catch (Exception e) {
            throw new RuntimeException(e);
        }

        seriesRepo.save(tvShow);
        return new TVShowSaveResultDto(
                tvShow.getId(),
                false,
    ...
    // ---- Get all series ----
    public List<TVShowListDto> getAllSeries() {
        return seriesRepo.findAll(Sort.by(Sort.Direction.DESC, "updatedAt"))
                .stream().map(TVShowListDto::fromEntity).toList();
    }

    // ---- Get series by id ----
    public TVShowDto getSeriesById(Long id) {
        Optional<TVShow> seriesOpt = seriesRepo.findById(id);
        TVShow series = seriesOpt.orElseThrow(() ->
    ...
    private SyncResultDto syncLibrarySeries(Long id) {
        TVShowDto localTVSeries = getSeriesById(id);
        ...
        TVShow seriesToSave = TVShowDto.toEntity(localTVSeries);
